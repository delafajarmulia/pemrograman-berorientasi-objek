...
class LogAir:
    ...
    def __init__(self, log_id: int, slot_id: int, tanggal_cek: datetime.date,
                 ppm_aktual: int, ph_aktual: float):
        self.__id = log_id
    ...
    def status_ph(self) -> str:
        """Abstraksi logika penentuan status kesehatan pH air baku."""
...
class SlotTanam:
    """Model data utama yang menerapkan prinsip komposisi terhadap LogAir."""

    def __init__(self, slot_id: int, nama_slot: str, jenis_tanaman: str,
                 tanggal_tanam: datetime.date, volume_air: float):
        self.__id = slot_id
        self.nama_slot = nama_slot
        self.jenis_tanaman = jenis_tanaman
        self.tanggal_tanam = tanggal_tanam
        self.volume_air_liter = volume_air
        self.daftar_log: list[LogAir] = []  # Prinsip Komposisi
    ...
    def hitung_umur_hst(self) -> int:
        ...
        return max(0, selisih.days)

    def tentukan_fase(self) -> str:
        """Mengklasifikasikan fase pertumbuhan tanaman."""
